...
function dockerComposeToApi(input) {
  ...
  //  const validation = Joi.validate(input, serviceSchema);
  const validation = serviceSchema.validate(input);
  ...
  if ('deploy' in input[name]) {
    ...
    const deployMode = ('mode' in input[name].deploy) ? input[name].deploy.mode : 'replicated';
    const modeOptions = {};
