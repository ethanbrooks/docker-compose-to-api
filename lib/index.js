...
function dockerComposeToApi(input) {
  ...
  if ('deploy' in input[name]) {
    ...
    if ('mode' in input[name]['deploy']) {
      var options = {};
      if ('replicas' in input[name]['deploy']) options.Replicas = parseInt(input[name]['deploy']['replicas'], 10);
      spec.Mode = require('./deploy.mode.js')(input[name]['deploy']['mode'], options);
    }
